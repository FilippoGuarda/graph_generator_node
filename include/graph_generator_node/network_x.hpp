 ...
 * Stores nodes and directed edges with path and weight metadata
 */
class NetworkX {
 public:
  struct Node {
  ...
  // Edge operations - Accept both std::pair and cv::Point2i vectors
  
  // Version 1: std::pair<int, int> vector
  ...
  // Version 2: cv::Point2i vector (template for type safety)
  void addEdge(int from, int to, const std::vector<cv::Point2i>& path, double weight) {
    long long key = makeEdgeKey(from, to);
 ...
 private:
  ...
  static long long makeEdgeKey(int from, int to) {
    ...
    return (static_cast<long long>(from) << 32) | static_cast<uint32_t>(to);
  }
};
